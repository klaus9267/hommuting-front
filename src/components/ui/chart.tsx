...
type ChartTooltipContentProps = {
  ...
  formatter?: (value: number | string, name: string, item: any, index: number, payload: any) => React.ReactNode;
  color?: string;
  nameKey?: string;
  labelKey?: string;
};
...
function ChartTooltipContent({ active, payload, className, indicator = 'dot', hideLabel = false, hideIndicator = false, label, labelFormatter, labelClassName, formatter, color, nameKey, labelKey }: ChartTooltipContentProps) {
  ...
  const tooltipLabel = React.useMemo(() => {
    ...
    if (labelFormatter) {
      return <div className={cn('font-medium', labelClassName)}>{labelFormatter(value as any, payload as any)}</div>;
    }
              ...
              {formatter && item?.value !== undefined && item.name ? (
                ...
                <>
                  ...
                  <div className={cn('flex flex-1 justify-between leading-none', nestLabel ? 'items-end' : 'items-center')}>
                    ...
                    {(item as any)?.value && <span className="text-foreground font-mono font-medium tabular-nums">{(item as any).value.toLocaleString()}</span>}
                  </div>
                </>
              )}
            </div>
  ...
  return (
    <div className={cn('flex items-center justify-center gap-4', verticalAlign === 'top' ? 'pb-3' : 'pt-3', className)}>
      {(payload as any[]).map(item => {
        ...
        return (
          <div key={(item as any).value} className={'[&>svg]:text-muted-foreground flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3'}>
