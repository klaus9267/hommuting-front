...
export function PropertyFilters() {
  ...
  return (
    <>
        ...
        <div className="py-4 px-[2%] space-y-4">
          ...
          <div className="flex items-center gap-2">
            {propertyTypes.map(type => (
              <Button key={type.id} variant={activeType === type.id ? 'default' : 'outline'} size="sm" onClick={() => setActiveType(type.id)} className="h-9">
                {type.label}
              </Button>
            ))}
          </div>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            {detailFilters.map(filter => {
              ...
              return (
                <Button key={filter.id} variant={hasAppliedFilter ? 'default' : 'outline'} size="sm" className="h-9 gap-2 text-muted-foreground hover:text-foreground" onClick={() => openFilterModal(filter.id as any)}>
                  <Icon className="w-4 h-4" />
                  <span className="text-sm">{filter.label}</span>
                  <span className={`text-sm font-medium ${hasAppliedFilter ? 'text-primary-foreground' : 'text-foreground'}`}>{getFilterDisplayValue(filter.id)}</span>
      ...
      <PropertyFilterModal isOpen={modalOpen} onClose={closeFilterModal} filterType={activeFilter} onApply={applyFilter} />
    </>
  );
}
